...
extension DemoViewModel {
    ...
    ///
    /// One ``ConvertSwiftSDK/on(_:callback:)`` subscription per `SystemEvent.allCases`
    ...
    /// cannot `await` the async ``ConvertSwiftSDK/off(_:)``).
    ///
    ...
    ///
    /// Calls ``ConvertSwiftSDK/off(_:)`` (idempotent) for each token from
