...
extension DemoViewModel {
    ...
    /// The maximum time ``start()`` waits for ``ConvertSwiftSDK/ready()`` before declaring the
    ...
    /// Drives the ``ConfigState`` machine: awaits ``ConvertSwiftSDK/ready()`` directly while a
    ...
    /// `.task` and `ConvertSwiftSDKDemoApp.swift` stay untouched), BEFORE the await so the Offline
    ...
    /// `ConvertSwiftSDKDemoApp.swift` `.task { await viewModel.start() }` call site compiles unchanged.
    func start() async {
    ...
    /// Applies a runtime tracking toggle: calls ``ConvertSwiftSDK/setTrackingEnabled(_:)``, then
    /// reads back ``ConvertSwiftSDK/isTrackingEnabled()`` to confirm, and publishes the result to
