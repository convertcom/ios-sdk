...
///
/// Owns the single ``ConvertSwiftSDK`` instance (keeping the SDK out of the App struct and any View's
...
final class DemoViewModel: ObservableObject {
    ...
    ///
    /// `ConvertSwiftSDK` is `final class … Sendable`, so it is held directly with no
    ...
    /// public config-snapshot accessor on ``ConvertSwiftSDK`` — the Config panel accessors (in
    ...
    /// ``ConvertSwiftSDK/isTrackingEnabled()`` read-back. `internal` setter — writer is cross-file.
