...
- **Per-call:** pass `enableTracking: false` to `runExperience(_:enableTracking:)` / `runExperiences(enableTracking:)` to bucket without emitting that one exposure event, or to `runFeature(_:enableTracking:experienceKeys:)` / `runFeatures(enableTracking:experienceKeys:)` to do the same for a feature's carrying experience. On the feature path this suppresses only the outbound exposure — the sticky decision and the in-process `.bucketing` event still fire, which is the opposite of the Android SDK. A preview target on the context always wins over the caller's value. `experienceKeys` further narrows which experiences a feature call may bucket at all; an excluded one produces no decision, no sticky write, and no exposure.
