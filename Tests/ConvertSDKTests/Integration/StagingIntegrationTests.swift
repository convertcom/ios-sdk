// Tests/ConvertSDKTests/Integration/StagingIntegrationTests.swift
//
...
//     — through the PUBLIC FR7 direct-data path (`ConvertSDK(configData:)`), with NO network at all.
//
...
//
//   1. The PUBLIC `ConvertSDK(configData:)` path validates the bytes but does NOT decode them into a
...
//
// ── Isolation (bd-ilx) ────────────────────────────────────────────────────────────────────────────
// The PUBLIC `ConvertSDK(configData:)` init has no `DecisionStore` injection seam (that is the internal
...
struct StagingIntegrationTests {
    ...
    /// `ConvertSDK(configData:)` → `ready()` → `createContext` → `runExperience`. Asserts the SDK
    ...
    func stagingStaticMode() async throws {
        ...
        let sdk = ConvertSDK(configData: data)
        try await sdk.ready()
    ...
    func stagingLiveMode() async throws {
        ...
        let sdk = ConvertSDK(configuration: configuration)
        try await sdk.ready()
