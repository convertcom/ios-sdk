// Tests/ConvertSDKTests/ConversionTrackingTests.swift
// `@testable import ConvertSDK` (the established pattern — see `ConvertContextTests.swift` /
...
// The injection seam does not exist: `ConvertSDK.init` has NO `eventSink:` / `logger:` parameter, so
// the `makeReadySDK` factory's `ConvertSDK(... eventSink:, logger:)` call FAILS TO COMPILE — that
...
//   * `ConvertSDK.init(... eventSink: any EventSink = NoopEventSink(), logger: any Logger = NoopLogger())`
...
//   * `ConvertSDK` stores `eventSink` + `logger` and passes them (with its existing private `eventBus`)
...
struct ConversionTrackingTests {
    ...
    /// read collaborators by name. `Sendable` — `ConvertSDK` is `Sendable`, `MockEventSink` is an
    ...
    ///
    /// ASSUMES THE GREEN SEAM: the `eventSink:` and `logger:` parameters do not exist on `ConvertSDK.init`
