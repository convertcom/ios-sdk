// Tests/ConvertSDKTests/Lifecycle/LifecycleObserverTests.swift
//
...
// `Sources/ConvertSDK/Lifecycle/LifecycleObserver.swift`. Every reference here goes through the
...
// `EventQueue.flush()` is `private` and `persistBeforeBackground()` is `internal` to ConvertSDKCore
// — NEITHER is reachable from the `ConvertSDK` target where the observer lives. So these tests assert
