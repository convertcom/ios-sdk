// Tests/ConvertSDKTests/GoalDeduplicationTests.swift
...
// `ConvertSDK(...)` call. The recovered-event read-back is the shared file-private `conversionData(from:)`
...
struct GoalDeduplicationTests {
    ...
    /// struct (not a large tuple) keeps the `large_tuple` lint rule satisfied. `Sendable` — `ConvertSDK` is
    ...
    /// `MockFileStore`), returning the SDK and its fresh sink. Single owner of the `ConvertSDK(...)` + `ready()`
    /// build so neither launch in the cross-launch test re-inlines it (SonarQube 3% gate).
    private func makeSdk(store: DecisionStore) async throws -> (sdk: ConvertSDK, sink: MockEventSink) {
        let sink = MockEventSink()
        let sdk = ConvertSDK(
            configuration: ConvertConfiguration(sdkKey: "test-key"),
            configProvider: MockConfigProvider.ungated(
