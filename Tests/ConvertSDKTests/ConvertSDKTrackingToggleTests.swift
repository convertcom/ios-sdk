// Tests/ConvertSDKTests/ConvertSDKTrackingToggleTests.swift
//
// Story 5.6 (Epic 5) RED phase: tests for the RUNTIME tracking toggle API added to `ConvertSDK`.
//
// References the NOT-YET-EXISTING public methods:
//   • `ConvertSDK.setTrackingEnabled(_ enabled: Bool) async`
//   • `ConvertSDK.isTrackingEnabled() async -> Bool`
//
...
// cycle. Every other call site (ConvertSDK.init, ready(), createContext, runExperience,
...
// `@testable import ConvertSDK` reaches `internal` init seams (MockFileStore, DecisionStore
// injection). `MockEventSink`, `MockLogger`, `MockConfigProvider`, `MockFileStore`, and `MockLogger`
// are reused from `Tests/ConvertSDKTests/Support/MockPorts.swift` — NOT redeclared here.
import Testing
import Foundation
...
/// Story 5.6 (Epic 5) RED-phase test suite for `ConvertSDK.setTrackingEnabled(_:)` /
...
/// methods to `ConvertSDK`. All other surface — `ready()`, `createContext`, `runExperience`,
...
@MainActor
struct ConvertSDKTrackingToggleTests {
    ...
    /// `TrackingSUT` shape. `Sendable` — `ConvertSDK` is `Sendable`; `MockEventSink` is an `actor`.
