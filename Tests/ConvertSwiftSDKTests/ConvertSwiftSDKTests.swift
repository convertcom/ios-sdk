// Tests/ConvertSwiftSDKTests/ConvertSwiftSDKTests.swift
// `@testable` import: these suites construct `ConvertSwiftSDK` through its INTERNAL
...
//
// RED phase ([SDK-2] — wiring the real config fetch into `ConvertSwiftSDK.init`): the GREEN
...
// surface — `ConvertSwiftSDK(...)` / `ready()` / `on` / `off` / `createContext` /
...
@MainActor
private func makeSut(sdkKey: String = validSdkKey) throws -> ConvertSwiftSDK {
    makeSut(
        sdkKey: sdkKey,
...
/// `Tests/ConvertSwiftSDKCoreTests/Event/EventBusTests.swift`.
...
@Suite("ConvertSwiftSDK init")
struct ConvertSwiftSDKInitTests {
    ...
    /// `ConvertSwiftSDK.shared` is `nil` until an explicit opt-in assignment — construction alone
    /// must not install a global singleton.
    @Test("shared is nil by default")
    func sharedIsNilByDefault() {
        #expect(ConvertSwiftSDK.shared == nil)
    }
}
...
@Suite("ready()")
struct ConvertSwiftSDKReadyTests {
...
struct ConvertSwiftSDKOnOffTests {
...
@Suite("createContext")
struct ConvertSwiftSDKCreateContextTests {
...
struct ConvertSwiftSDKCompletionTests {
