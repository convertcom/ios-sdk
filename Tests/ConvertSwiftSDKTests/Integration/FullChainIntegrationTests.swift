// Tests/ConvertSwiftSDKTests/Integration/FullChainIntegrationTests.swift
//
// FR67 full-chain integration suite — the Epic 5 / Story 5 RELEASE-GATE wiring test. Drives the REAL
// public `ConvertSwiftSDK` API (createContext → runExperience → runFeature → setDefaultSegments →
...
// single `ConvertSwiftSDK` instance. That is IMPOSSIBLE test-only: `ConvertSwiftSDK` has NO URLSession injection
...
/// `Sendable` — `ConvertSwiftSDK` is `Sendable`, `EventQueue` is an `actor`, `URL`/`String` are value types.
...
final class FullChainIntegrationTests {
    ...
    /// Builds the full-chain SUT: a READY `ConvertSwiftSDK` whose config is supplied OUT-OF-BAND (the
