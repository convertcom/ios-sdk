...
// The `EventQueueStore` test double for the PLATFORM target (`ConvertSwiftSDKTests`). It is a sibling
...
// model, both re-exported through `ConvertSwiftSDK`.
//
// An identical double lives in `ConvertSwiftSDKCoreTests/Support/MockCorePorts.swift`, but the two test
// targets cannot share support code: `ConvertSwiftSDKTests` cannot see `ConvertSwiftSDKCoreTests`' types (and
