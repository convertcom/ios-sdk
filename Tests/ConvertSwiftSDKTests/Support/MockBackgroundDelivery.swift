...
// (`ConvertSwiftSDKTests`) needs but does not yet have: the background-upload seam double
...
// reference only the production ports (`EventUploader`, re-exported through `ConvertSwiftSDK`) and the
// to-be-built `BackgroundUploadEnqueueing` seam.
//
// ── Target-visibility note ──────────────────────────────────────────────────────────────────────
// A `MockEventUploader` already exists in `ConvertSwiftSDKCoreTests/Support/MockCorePorts.swift`, but the
// two test targets cannot share support code: `ConvertSwiftSDKTests` cannot see `ConvertSwiftSDKCoreTests`'
