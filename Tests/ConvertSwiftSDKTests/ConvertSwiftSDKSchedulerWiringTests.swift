// Tests/ConvertSwiftSDKTests/ConvertSwiftSDKSchedulerWiringTests.swift
//
// RED-phase contract for WIRING the `ConfigRefreshScheduler` into `ConvertSwiftSDK.init`
...
// The SUT helper below calls the internal initializer with a `clock:` argument. `ConvertSwiftSDK.init`
// has NO `clock:` parameter today (verified: `Sources/ConvertSwiftSDK/ConvertSwiftSDK.swift` declares
...
// `ConvertSwiftSDK` stays an all-`let` `Sendable final class` with NO `@unchecked`/`nonisolated(unsafe)`.
//
...
@Suite("ConvertSwiftSDK scheduler wiring")
struct ConvertSwiftSDKSchedulerWiringTests {
    // MARK: - SUT
    ...
    /// collaborators by name. `Sendable` — `ConvertSwiftSDK` is a `Sendable final class`, `MockClock`
    ...
    /// Builds a `ConvertSwiftSDK` wired to a COUNTING `MockConfigFetchService` (cache miss + a non-nil
    ...
    ///
    /// `@MainActor` to mirror the existing `ConvertSwiftSDKTests.makeSut` (the SDK's internal init is
