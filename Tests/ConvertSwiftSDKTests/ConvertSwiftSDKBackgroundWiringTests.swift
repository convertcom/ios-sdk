// Tests/ConvertSwiftSDKTests/ConvertSwiftSDKBackgroundWiringTests.swift
//
// RED-phase contract for WIRING durable background delivery (Epic 5 / Story 5.3 — PLAT-4 wiring
// phase) into `ConvertSwiftSDK`, and for the public `handleEventsForBackgroundURLSession(identifier:
...
// (and a `LifecycleObserver` over it) inside `ConvertSwiftSDK`, holds the manager on an internal property
...
// and read `sdk.backgroundSessionManager?.backgroundCompletionHandler`. `ConvertSwiftSDK` (verified:
// `Sources/ConvertSwiftSDK/ConvertSwiftSDK.swift`) declares NEITHER today — so this file fails to compile with
// "value of type 'ConvertSwiftSDK' has no member 'handleEventsForBackgroundURLSession'" and "… has no member
...
// `makeRefreshConfig` (Support/TestFixtures.swift), `ConvertConfiguration`, `ConvertSwiftSDK`'s internal
// init + `ready()`, AND `BackgroundSessionManager.sessionIdentifier` (an EXISTING `static let` on the
// manager, reachable here through `@testable import ConvertSwiftSDK`). The compile-fail is isolated to the
...
// non-counting `MockConfigProvider` (the same double `ConvertSwiftSDKTests.makeSut` injects) is the lightest
...
// `ConvertSwiftSDKSchedulerWiringTests` to prove the refresh loop ticks) would add machinery this suite never
...
@Suite("ConvertSwiftSDK background wiring")
struct ConvertSwiftSDKBackgroundWiringTests {
    // MARK: - SUT

    /// Builds a REAL `ConvertSwiftSDK` wired to an UNGATED `MockConfigProvider` (cache miss + a non-nil
    ...
    ///
    /// `@MainActor` to mirror the existing wiring SUTs (`ConvertSwiftSDKTests.makeSut` /
    /// `ConvertSwiftSDKSchedulerWiringTests.makeWiringSut`): the SDK's internal init is non-async (the handle
    ...
    @MainActor
    private func makeSut() async throws -> ConvertSwiftSDK {
        ...
        let sdk = ConvertSwiftSDK(configuration: configuration, configProvider: provider)
    ...
    /// PLAT-4 wiring smoke (composition-root proof): a standard-key-path `ConvertSwiftSDK` constructs with a
