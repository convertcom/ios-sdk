// MockCorePorts.swift
// Test-double conformances for the ConvertSwiftSDKCore *persistence* ports consumed by the
// Epic 3 / Story 1 visitor-context suite: `SecureStore`, `KeyValueStore`, and `Logger`.
//
// ── Why a SEPARATE file from Tests/ConvertSwiftSDKTests/Support/MockPorts.swift ─────────────
// The two test targets cannot share code: `ConvertSwiftSDKCoreTests` depends ONLY on
// `ConvertSwiftSDKCore` (Package.swift), while `MockPorts.swift` lives in `ConvertSwiftSDKTests`
// (which depends on `ConvertSwiftSDK`) and is therefore invisible here. The mocks below
// conform to the *Core* port protocols (`SecureStore`/`KeyValueStore`/`Logger`), which are
// reachable through the `ConvertSwiftSDKCore` dependency. `MockLogger` is re-declared here for
// the same visibility reason — the `ConvertSwiftSDKTests` one cannot be seen from this target.
//
...
// Core-port mock aggregator for the whole `ConvertSwiftSDKCoreTests` target — persistence ports
...
// `file_length` disable convention in `Tests/ConvertSwiftSDKTests/Support/TestFixtures.swift`.
...
/// Test double for ``Logger``. Re-declared here (not shared from `ConvertSwiftSDKTests`) because
...
/// Epic 3 / Story 2 bucketing suite). Re-declared here (not shared from `ConvertSwiftSDKTests`)
/// for the same target-visibility reason as ``MockLogger`` above — `ConvertSwiftSDKCoreTests`
/// cannot see the `ConvertSwiftSDKTests` copy in `MockPorts.swift`.
///
...
/// through). Re-declared here (not shared from `ConvertSwiftSDKTests`) for the same
/// target-visibility reason as ``MockLogger`` / ``MockEventSink`` above — `ConvertSwiftSDKCoreTests`
/// cannot see the `ConvertSwiftSDKTests` copy in `MockPorts.swift`. Mirrors that copy's API
...
/// `ConvertSwiftSDKTests/MockPorts.swift` — for the same target-visibility reason as
/// ``MockEventSink`` above: `ConvertSwiftSDKCoreTests` cannot see the `ConvertSwiftSDKTests` copy,
...
/// target — not shared from `ConvertSwiftSDKTests/MockPorts.swift` — for the same target-visibility
/// reason as ``MockEventUploader`` / ``MockEventSink`` above: `ConvertSwiftSDKCoreTests` cannot see the
/// `ConvertSwiftSDKTests` copy, and `EventQueueTests` lives here.
///
