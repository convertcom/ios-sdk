...
// Foundation-only — part of the pure-logic ConvertSwiftSDKCore target (no UIKit/AppKit/Security/
...
public actor EventQueue: EventSink {
    ...
    /// Mutable so `ConvertSwiftSDK.setTrackingEnabled(_:)` can flip the runtime gate without
    ...
    ///
    /// `package` (NOT `public`) — only `ConvertSwiftSDK.setTrackingEnabled(_:)` in the same
    ...
    /// SDK consumers reach the toggle only through the public ``ConvertSwiftSDK`` API.
    ...
    ///
    /// `package` (NOT `internal`): the `LifecycleObserver` lives in the sibling `ConvertSwiftSDK` target
    ...
    /// ``VisitorContextManager``, both already consumed cross-target by `ConvertSwiftSDK`.
    ...
    ///
    /// `package` (NOT `private`): the `LifecycleObserver` in the sibling `ConvertSwiftSDK` target drives
