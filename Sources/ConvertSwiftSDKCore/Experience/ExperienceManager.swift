...
/// `ExperienceManagerTests`): the cross-module ``ConvertSwiftSDK`` target stores this type on its
...
public struct ExperienceManager: Sendable {
    ...
    /// ONE public entry point a cross-module caller (``ConvertSwiftSDK``) uses, so it never touches the
    ...
    ///     production composition root (``ConvertSwiftSDK``, a later task) threads the real `EventQueue`
    ...
    ///     but the production composition root (``ConvertSwiftSDK``, a later task) injects the real
