...
// persist a fresh one. Foundation-only — part of the pure-logic ConvertSwiftSDKCore target.
...
///
/// `package` (not `internal` or `public`) because the resolver lives in ``ConvertSwiftSDKCore`` but its
/// sole caller is ``ConvertSwiftSDK/createContext(visitorId:attributes:)`` in the SEPARATE `ConvertSwiftSDK`
...
/// `@_exported import ConvertSwiftSDKCore` re-export. The architecture mandates the public surface be only
/// `ConvertSwiftSDK`/`ConvertContext`/`ConvertConfiguration`/model-DTOs — everything else is
