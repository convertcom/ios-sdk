...
- **Don't initialize.** If a user has not consented, simply do not construct the SDK. With no ``ConvertSwiftSDK/init(configuration:)`` call there is no identifier, no bucketing, and no event.
...
- **Disable delivery for a single decision (per-call).** Pass `enableTracking: false` to ``ConvertContext/runExperience(_:enableTracking:)`` to bucket a visitor without emitting that exposure event. The **feature** path takes the same per-call `enableTracking` (``ConvertContext/runFeature(_:enableTracking:experienceKeys:)``, CAP-1), but there it suppresses only the outbound bucketing exposure — the sticky decision is still written and the in-process ``SystemEvent/bucketing`` signal still fires, because sticky bucketing is a correctness invariant, not a tracking concern. **This is the opposite of the Android SDK**, which suppresses its in-process fire too. On both paths, a preview target set on the context always wins: `enableTracking: true` cannot re-enable delivery or persistence once the context is previewing. To keep a carrying experience from bucketing at all — no decision, no sticky write, no exposure — exclude its key via `experienceKeys` (CAP-2) rather than relying on `enableTracking` alone.
- **Disable delivery at runtime (whole SDK, mid-session).** Call ``ConvertSwiftSDK/setTrackingEnabled(_:)`` at any point after initialization. `await sdk.setTrackingEnabled(false)` stops NEW bucketing and conversion events from being collected; events already buffered before the call still flush on the next delivery cycle. `await sdk.setTrackingEnabled(true)` re-opens delivery (suppressed events are not replayed). ``ConvertSwiftSDK/isTrackingEnabled()`` reads the current state. This is the supported consent-withdrawal path for GDPR mid-session opt-out — the SDK ships the control; you own the decision.
