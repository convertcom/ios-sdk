...
A silent network hang during the initial configuration fetch does not announce itself — ``ConvertSwiftSDK/ready()`` stays suspended, and any UI gated on it spins forever. The fix is to bound the wait so a hang flips into a visible failure instead of an infinite spinner.

Apply a **10-second readiness timeout**: race ``ConvertSwiftSDK/ready()`` against a timeout and treat whichever finishes first as the outcome. The ready/timeout pattern is in <doc:FailureDetection>.
...
- **The SDK is not ready yet** — `runExperience` was called before ``ConvertSwiftSDK/ready()`` resolved, so no config was in memory to decide against.
