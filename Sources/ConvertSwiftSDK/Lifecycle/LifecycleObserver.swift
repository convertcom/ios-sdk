...
// Durable background-delivery lifecycle observer (Epic 5 / Story 5.3). Lives in the `ConvertSwiftSDK`
// (platform) target — NOT the pure-logic `ConvertSwiftSDKCore` — because it observes app-lifecycle
...
/// `@unchecked Sendable` and NO `nonisolated(unsafe)` — mirroring `ConvertSwiftSDK`'s `Sendable` proof via
...
final class LifecycleObserver: Sendable {
    ...
    /// (`Sendable`) ``notificationCenter`` — never `self` — mirroring `ConvertSwiftSDK.deinit`'s handoff
