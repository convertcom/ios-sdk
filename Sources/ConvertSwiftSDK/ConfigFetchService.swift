...
// on-disk cache. Lives in the `ConvertSwiftSDK` (platform) target because it composes the
// Foundation-backed `CoordinatedFileStore`; the ports it depends on (`HTTPClient`,
// `Logger`) are Foundation-only and live in the pure-logic `ConvertSwiftSDKCore`.
...
/// ``loadCachedConfig()`` / ``fetchLiveConfig()`` satisfy the seam `ConvertSwiftSDK.init` injects,
