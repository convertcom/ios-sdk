...
//
// Placement rationale: in the `ConvertSwiftSDK` platform target (NOT `ConvertSwiftSDKCore`) because:
//   (a) `EventQueue` propagation happens via a downcast in `ConvertSwiftSDK.setTrackingEnabled`
...
//   (c) the `TrackingState` reference is held as a `let` on `ConvertSwiftSDK`, so the class stays
//       an all-`let` Sendable final class with NO `@unchecked Sendable`.

/// Actor-isolated runtime tracking flag.
///
/// Owns the single mutable bit that `ConvertSwiftSDK.setTrackingEnabled(_:)` flips and
/// `ConvertSwiftSDK.isTrackingEnabled()` reads. Held as a `let` on `ConvertSwiftSDK`, so the SDK
...
actor TrackingState {
    ...
    /// Replaces the tracking flag. Called by `ConvertSwiftSDK.setTrackingEnabled(_:)`.
