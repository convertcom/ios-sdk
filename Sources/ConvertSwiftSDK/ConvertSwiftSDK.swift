// ConvertSwiftSDK.swift
// Public entry point for the Convert iOS SDK (Epic 2 / Story 2).
// Re-exports ConvertSwiftSDKCore so consumers need only `import ConvertSwiftSDK`.
//
...
@_exported import ConvertSwiftSDKCore
import Foundation
...
/// `@unchecked Sendable`.
public final class ConvertSwiftSDK: Sendable {
    ...
    /// background session), so this `let` keeps ``ConvertSwiftSDK`` an all-`let` `Sendable final class` with NO
    ...
    /// `Sendable final class`, so the `let` keeps ``ConvertSwiftSDK`` `Sendable` with no suppression.
    ...
    nonisolated(unsafe) public static var shared: ConvertSwiftSDK?

    /// Actor-isolated runtime tracking flag (Story 5.6). Held as a `let` so `ConvertSwiftSDK`
    ...
    internal init( // swiftlint:disable:this function_body_length
        ...
        let resolvedSink = ConvertSwiftSDK.resolveEventSink(
        ...
        // ConvertSwiftSDKCore, so this target never names those internal types. NoopLogger matches the SDK's
        ...
        Task {
            ...
            // `ConfigValidation` is internal to ConvertSwiftSDKCore (invisible to this target).
    ...
    ///
    /// The ``ConvertSwiftSDK`` handle remains an all-`let` `Sendable final class`: the mutable bit
