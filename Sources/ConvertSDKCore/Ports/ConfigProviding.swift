...
// Port: the config-fetch seam `ConvertSDK.init` injects (Epic 2 / Story 3).
// Foundation-only — part of the pure-logic ConvertSDKCore target.

import Foundation

/// Port: supplies the project configuration to ``ConvertSDK`` from two sources — the on-disk
...
/// every error internally), so `ConvertSDK.init`'s load task never has to catch — it calls
