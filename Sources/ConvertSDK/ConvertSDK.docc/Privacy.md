...
The SDK performs no cross-app tracking: there is no ATT prompt and the IDFA and IDFV are never read. Visitor identity is a persistent, app-scoped UUID. Event delivery can be disabled entirely through the configuration's `networkTracking` flag while bucketing continues to work.
