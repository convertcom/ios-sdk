...
- **Disable delivery for a single decision (per-call).** Pass `enableTracking: false` to ``ConvertContext/runExperience(_:enableTracking:)`` to bucket a visitor without emitting that exposure event. Note this per-call flag governs the **experience** path; on the **feature** path (``ConvertContext/runFeature(_:enableTracking:)``) it is currently a no-op, so use the static `networkTracking` flag when you need to suppress feature tracking.
