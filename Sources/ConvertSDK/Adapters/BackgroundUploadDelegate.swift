...
// the on-disk event queue. Lives in the `ConvertSDK` target because it composes the
// URLSession-backed background transport; the ports it drives (``EventQueueStore``) and the bus it
// fires on (``EventBus``) are Foundation-only and live in the pure-logic `ConvertSDKCore`.
