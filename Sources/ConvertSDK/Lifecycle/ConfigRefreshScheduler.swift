...
// (Epic 2 / Story 2.4). Lives in the `ConvertSDK` (platform) target — NOT the
// pure-logic `ConvertSDKCore` — because it observes app-lifecycle notifications
...
// Foundation-only and live in `ConvertSDKCore`.
...
public actor ConfigRefreshScheduler {
    ...
    /// scheduler composes the same injected provider `ConvertSDK` uses and a test injects a
