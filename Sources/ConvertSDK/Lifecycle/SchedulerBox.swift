...
// (Epic 2 / Story 2.4 — PLAT-2 wiring phase). Lives in the `ConvertSDK` (platform)
// target alongside `ConfigRefreshScheduler`, NOT in the pure-logic `ConvertSDKCore`.

import Foundation

/// Holds the lazily-started refresh scheduler behind an actor so ``ConvertSDK`` can own it through
...
/// ``ConvertSDK`` is a `Sendable final class` whose data-race safety the compiler proves from ALL
...
/// itself `Sendable` (every actor is), so ``ConvertSDK`` stores it as a `let`; the actor's OWN
...
///
/// ── Teardown ─────────────────────────────────────────────────────────────────────────────────
/// ``ConvertSDK/deinit`` cannot `await`, so it hands off to a detached `Task` that calls
